#ifndef CHROMATOGRAPHDATAPROCESSDIALOG_H
#define CHROMATOGRAPHDATAPROCESSDIALOG_H
...
// 前置声明
class DataNavigator;
class ChromatographParameterSettingsDialog;

namespace Ui {
class ChromatographDataProcessDialog;
}

class ChromatographDataProcessDialog : public QWidget
{
    Q_OBJECT

public:
    explicit ChromatographDataProcessDialog(QWidget *parent = nullptr, AppInitializer* appInitializer = nullptr, DataNavigator *mainNavigator = nullptr);
    ~ChromatographDataProcessDialog();
...
signals:
    ...
    void requestNewChromatographDifferenceWorkbench(int referenceSampleId,
...
private slots:
    ...
    // 主导航样本选中状态变化的槽函数（带数据类型），用于动态同步色谱导航树
...
private:
    ...
    // 参数设置对话框
    ChromatographParameterSettingsDialog* m_parameterDialog;
    ...
    QList<QVariantMap> m_chromatographTgSamples;
...
// 【升级】缓存也变成了一个 Map
    ...
    ChromatographParameterSettingsDialog *m_paramDialog = nullptr;
...
#endif // CHROMATOGRAPHDATAPROCESSDIALOG_H
