...
SingleMaterialDataWidget::~SingleMaterialDataWidget()
{
    ...
    if (m_tgSmallRawDataImportWorker) {
        if (m_tgSmallRawDataImportWorker->isRunning()) {
            m_tgSmallRawDataImportWorker->stop();
            m_tgSmallRawDataImportWorker->wait();
        }
        delete m_tgSmallRawDataImportWorker;
        m_tgSmallRawDataImportWorker = nullptr;
    }
...
void SingleMaterialDataWidget::on_importTgSmallRawDataButton_clicked()
{
    QString filePath = QFileDialog::getOpenFileName(this, tr("选择小热重（原始数据）文件"),
                                                   QDir::homePath(),
                                                   tr("Excel文件 (*.xlsx)"));
    if (filePath.isEmpty()) {
        emit statusMessage(tr("未选择文件，操作已取消。"), 3000);
    ...
    if (m_tgSmallRawDataImportWorker && m_tgSmallRawDataImportWorker->isRunning()) {
        m_tgSmallRawDataImportWorker->stop();
        m_tgSmallRawDataImportWorker->wait();
    }
    ...
    if (!m_importProgressDialog) {
        m_importProgressDialog = new QProgressDialog(this);
        m_importProgressDialog->setWindowModality(Qt::NonModal);
        m_importProgressDialog->setMinimumDuration(0);
        m_importProgressDialog->setCancelButtonText(tr("取消"));
        m_importProgressDialog->setAutoClose(false);
        m_importProgressDialog->setAutoReset(false);
    ...
    m_importProgressDialog->setWindowTitle(tr("导入小热重（原始数据）"));
    m_importProgressDialog->setLabelText(tr("准备导入数据..."));
    m_importProgressDialog->setValue(0);
    m_importProgressDialog->setRange(0, 100);
    m_importProgressDialog->show();
    ...
    if (!m_tgSmallRawDataImportWorker) {
        m_tgSmallRawDataImportWorker = new TgSmallRawDataImportWorker(this);

        connect(m_tgSmallRawDataImportWorker, &TgSmallRawDataImportWorker::progressChanged,
                this, [this](int current, int total) {
                    if (m_importProgressDialog) {
                        m_importProgressDialog->setRange(0, total);
                        m_importProgressDialog->setValue(current);
        ...
        connect(m_tgSmallRawDataImportWorker, &TgSmallRawDataImportWorker::progressMessage,
                this, [this](const QString& message) {
                    if (m_importProgressDialog) {
                        m_importProgressDialog->setLabelText(message);
                    }
        ...
        connect(m_tgSmallRawDataImportWorker, &TgSmallRawDataImportWorker::importFinished,
                this, [this](int successCount, int totalDataCount) {
                    if (m_importProgressDialog) {
                        m_importProgressDialog->close();
                    }
                    ...
                    if (successCount > 0) {
                        ...
                        refreshCharts();
                        emit dataImportFinished("TG_SMALL_RAW");
                    } else {
                        QString detailedMsg = tr("导入失败：未从Excel文件中提取到有效数据。\n\n可能的原因：\n"
                                              "1. Excel文件格式不正确\n"
                                              "2. 工作表名称不包含有效的样本编号\n"
        ...
        connect(m_tgSmallRawDataImportWorker, &TgSmallRawDataImportWorker::importError,
                this, [this](const QString& errorMessage) {
                    if (m_importProgressDialog) {
                        m_importProgressDialog->close();
                    }
        ...
        connect(m_importProgressDialog, &QProgressDialog::canceled,
                m_tgSmallRawDataImportWorker, &TgSmallRawDataImportWorker::stop);
    }
