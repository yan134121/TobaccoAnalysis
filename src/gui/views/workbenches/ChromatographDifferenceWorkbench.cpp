#include "ChromatographDifferenceWorkbench.h"
...
ChromatographDifferenceWorkbench::ChromatographDifferenceWorkbench(int referenceSampleId,
...
void ChromatographDifferenceWorkbench::setupUi()
{
    // --- 创建主布局 ---
...
// ===================================================================
void ChromatographDifferenceWorkbench::calculateAndDisplay()
{
    ...
    setWindowTitle(tr("色谱差异度分析 - 参考样本ID: %1").arg(m_referenceSampleId));
...
// ChromatographDifferenceWorkbench.cpp
void ChromatographDifferenceWorkbench::closeEvent(QCloseEvent* event)
{
    QMdiSubWindow::closeEvent(event);
    DEBUG_LOG << "ChromatographDifferenceWorkbench::closeEvent() emitted";
    emit closed(this);  // 通知外部
}


// 计算综合评分（使用界面权重参数进行加权）
double ChromatographDifferenceWorkbench::calculateComprehensiveScore(const QMap<QString, double>& scores)
{
...
// 导出到Excel
void ChromatographDifferenceWorkbench::exportToExcel(const QList<QMap<QString, QVariant>>& data, const QString& filePath)
{
...
// 显示最佳样品差异排序表（含归一化）
void ChromatographDifferenceWorkbench::onShowBestSampleRankingTable()
{
    // 创建对话框
    QDialog dialog(this);
    ...
    for (int i = 0; i < resultData.size(); ++i) {
        ...
        bool isOptimal = (pearsonRaw >= CHROMATOGRAPH_OPTIMAL_PEARSON_RAW_THRESHOLD
                          && pearsonNormalized >= CHROMATOGRAPH_OPTIMAL_PEARSON_NORMALIZED_THRESHOLD);
    ...
    connect(exportButton, &QPushButton::clicked, [this, resultData]() {
        QString filePath = QFileDialog::getSaveFileName(this, "保存Excel文件",
                                                      QDir::homePath() + "/色谱最佳样品差异排序表.xlsx",
...
//     QList<QSharedPointer<Curve>> allCurves;
//     for (auto it = m_processedData.constBegin(); it != m_processedData.constEnd(); ++it) {
//         QSharedPointer<Curve> derivativeCurve = it.value().derivative;
//         if (!derivativeCurve.isNull()) {
//             allCurves.append(derivativeCurve);
//             if (it.key() == m_referenceSampleId)
//                 referenceCurve = derivativeCurve;
//         }
//     }
...
//         double euclidean;
//     };
//     QMap<int, RawMetrics> rawMetricsMap; // sampleId -> 原始指标

//     double rmseMax = -std::numeric_limits<double>::infinity();
//     double euclideanMax = -std::numeric_limits<double>::infinity();
//     double pearsonMin = std::numeric_limits<double>::infinity();
//     double pearsonMax = -std::numeric_limits<double>::infinity();

//     for (auto curve : allCurves) {
//         int sampleId = curve->sampleId();
//         RawMetrics metrics;
//         metrics.rmse = comparer->calculateRMSE(referenceCurve, curve);
//         metrics.pearson = comparer->calculatePearsonCorrelation(referenceCurve, curve);
//         metrics.euclidean = comparer->calculateEuclideanDistance(referenceCurve, curve);

//         rawMetricsMap[sampleId] = metrics;

//         rmseMax = std::max(rmseMax, metrics.rmse);
//         euclideanMax = std::max(euclideanMax, metrics.euclidean);
//         pearsonMin = std::min(pearsonMin, metrics.pearson);
//         pearsonMax = std::max(pearsonMax, metrics.pearson);
//     }
...
//         RawMetrics metrics = rawMetricsMap[sampleId];

//         // 做归一化
//         double rmseNormalized = 1.0 - metrics.rmse / rmseMax;
//         double euclideanNormalized = 1.0 - metrics.euclidean / euclideanMax;
//         double pearsonNormalized = 0.0;
//         if (qAbs(pearsonMax - pearsonMin) > std::numeric_limits<double>::epsilon()) {
//             pearsonNormalized = (metrics.pearson - pearsonMin) / (pearsonMax - pearsonMin);
//         } else {
//             pearsonNormalized = 1.0; // 所有值相等时视为最优
//         }
...
//         // 添加到结果列表
//         QMap<QString, QVariant> rowData;
//         rowData["sample_id"] = sampleId;
//         rowData["sample_prefix"] = samplePrefix;
//         rowData["comprehensive_score"] = QString::number(comprehensiveScore, 'f', 9);
//         rowData["rmse_normalized"] = QString::number(rmseNormalized, 'f', 9);
//         rowData["pearson_normalized"] = QString::number(pearsonNormalized, 'f', 9);
//         rowData["euclidean_normalized"] = QString::number(euclideanNormalized, 'f', 9);
//         rowData["score_value"] = comprehensiveScore;
//         resultData.append(rowData);
//     }

//     // 按综合评分排序
//     std::sort(resultData.begin(), resultData.end(), [](const QMap<QString, QVariant>& a, const QMap<QString, QVariant>& b) {
//         return a["score_value"].toDouble() > b["score_value"].toDouble();
//     });
...
//     for (int i = 0; i < resultData.size(); ++i) {
//         resultData[i]["rank"] = i + 1;
//         bool isOptimal = resultData[i]["pearson_normalized"].toString().toDouble() >= 0.9
//                          && resultData[i]["rmse_normalized"].toString().toDouble() >= 0.9
//                          && resultData[i]["euclidean_normalized"].toString().toDouble() >= 0.9;
//         resultData[i]["is_optimal"] = isOptimal;
//     }
...
//         tableWidget->setItem(i, 0, new QTableWidgetItem(resultData[i]["sample_prefix"].toString()));
//         tableWidget->setItem(i, 1, new QTableWidgetItem(resultData[i]["comprehensive_score"].toString()));
//         tableWidget->setItem(i, 2, new QTableWidgetItem(resultData[i]["rmse_normalized"].toString()));
//         tableWidget->setItem(i, 3, new QTableWidgetItem(resultData[i]["pearson_normalized"].toString()));
//         tableWidget->setItem(i, 4, new QTableWidgetItem(resultData[i]["euclidean_normalized"].toString()));
//         tableWidget->setItem(i, 5, new QTableWidgetItem(QString::number(resultData[i]["rank"].toInt())));
//         tableWidget->setItem(i, 6, new QTableWidgetItem(resultData[i]["is_optimal"].toBool() ? "是" : "否"));
