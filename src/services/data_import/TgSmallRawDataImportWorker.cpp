...
void TgSmallRawDataImportWorker::run()
{
    ...
    if (temperatureColumn < 0 || tgColumn <= 0) {
    ...
    for (const QString& sheetName : sheetNames) {
        ...
        while (emptyRowCount < maxEmptyRows && row < 1000) {
            ...
            double tgValue = cellTg->value().toDouble(&ok2);
