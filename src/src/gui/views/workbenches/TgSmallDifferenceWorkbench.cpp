#include "TgSmallDifferenceWorkbench.h"
...
TgSmallDifferenceWorkbench::TgSmallDifferenceWorkbench(int referenceSampleId,
...
void TgSmallDifferenceWorkbench::setupUi()
{
    // --- 创建主布局 ---
    ...
    connect(m_tableView->selectionModel(), &QItemSelectionModel::currentRowChanged,
            this, &TgSmallDifferenceWorkbench::onResultTableRowClicked);
    connect(m_bestSampleRankingButton, &QPushButton::clicked,
            this, &TgSmallDifferenceWorkbench::onShowBestSampleRankingTable);
}
...
void TgSmallDifferenceWorkbench::onResultTableRowClicked(const QModelIndex &index)
{
...
// TgSmallDifferenceWorkbench.cpp
void TgSmallDifferenceWorkbench::closeEvent(QCloseEvent* event)
{
    QMdiSubWindow::closeEvent(event);
    DEBUG_LOG << "TgSmallDifferenceWorkbench::closeEvent() emitted";
    emit closed(this);  // 通知外部
}


// 计算综合评分（仅用归一化指标，权重1:1:1）
double TgSmallDifferenceWorkbench::calculateComprehensiveScore(const QMap<QString, double>& scores)
{
    ...
    //     DEBUG_LOG << "Total weight is too small, falling back to simple average.";
    //     // 权重总和为0时，回退到等权平均
    //     return (rmseNormalized + pearsonNormalized + euclideanNormalized) / 3.0;
    // }
    ...
    DEBUG_LOG << "Weighted comprehensive score:" << (weighted / totalWeight);
    return weighted / totalWeight;
}


// 导出到Excel
void TgSmallDifferenceWorkbench::exportToExcel(const QList<QMap<QString, QVariant>>& data, const QString& filePath)
{
...
// 显示最佳样品差异排序表（含归一化）
void TgSmallDifferenceWorkbench::onShowBestSampleRankingTable()
{
    // DEBUG_LOG << "TgSmallDifferenceWorkbench::onShowBestSampleRankingTable()";
    // 创建对话框
    QDialog dialog(this);
    ...
    // DEBUG_LOG << "m_processedData keys:" << m_processedData.keys();
    ...
    for (int i = 0; i < resultData.size(); ++i) {
        ...
        bool isOptimal = (pearsonRaw >= TG_SMALL_OPTIMAL_PEARSON_RAW_THRESHOLD
                          && pearsonNormalized >= TG_SMALL_OPTIMAL_PEARSON_NORMALIZED_THRESHOLD);
    ...
    connect(exportButton, &QPushButton::clicked, [this, resultData]() {
        QString filePath = QFileDialog::getSaveFileName(this, "保存Excel文件",
                                                      QDir::homePath() + "/小热重最佳样品差异排序表.xlsx",
